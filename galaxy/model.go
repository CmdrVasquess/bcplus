...
func (rpo *Repo) GetSystem(id int64, reuse *System) (*System, error) {
	if res, ok := rpo.chSysId[id]; ok {
		return res, nil
	}
	if reuse == nil {
		reuse = &System{Entity: Entity{r: rpo, Id: id}}
	} else {
		reuse.r = rpo
		reuse.Id = id
		reuse.parts = nil
	...
	err := row.Scan(&reuse.Name, &reuse.Coos[Xk], &reuse.Coos[Yk], &reuse.Coos[Zk])
	if err != nil {
